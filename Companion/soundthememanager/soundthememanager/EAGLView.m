...
{
	m_frames++;
	[renderer setXMPlayer:m_player];
    [renderer render];
}

- (void)logFLS:(id)sender
{
	NSLog(@"FLS: %d", m_frames);
	m_frames = 0;
}
...
- (void)startAnimation
{
    if (!animating)
    {
		...
		flsTimer = [NSTimer scheduledTimerWithTimeInterval:(NSTimeInterval)((1.0) * animationFrameInterval) target:self selector:@selector(logFLS:) userInfo:nil repeats:TRUE];
